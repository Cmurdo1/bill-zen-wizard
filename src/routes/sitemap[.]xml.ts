...
export const Route = createFileRoute("/sitemap.xml")({
  server: { handlers: { GET: async () => {
    const entries = [
      { path: "/", priority: "1.0" }, { path: "/pricing", priority: "0.9" }, { path: "/mcp", priority: "0.7" },
      { path: "/blog", priority: "0.8" }, ...SEO_PATHS.map((path) => ({ path, priority: "0.8" })),
      { path: "/pay-invoice", priority: "0.5" }, { path: "/pitch", priority: "0.4" }, { path: "/privacy", priority: "0.2" }, { path: "/terms", priority: "0.2" },
      ...BLOG_POSTS.map((p) => ({ path: `/blog/${p.slug}`, priority: "0.7", lastmod: p.date })),
    ];
    const urls = entries.map((e) => [`  <url>`, `    <loc>${BASE_URL}${e.path}</loc>`, e.lastmod ? `    <lastmod>${e.lastmod}</lastmod>` : null, `    <priority>${e.priority}</priority>`, `  </url>`].filter(Boolean).join("\n"));
